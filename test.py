...
ros = roslibpy.Ros('192.168.12.20', port=9090)
ros.run()

ros.on_ready(lambda: print('connected', ros.is_connected))

jm = roslibpy.Service(ros, '/mirsupervisor/setRobotState', 'mir_srvs/JoystickMode')
req = roslibpy.ServiceRequest(values=roslibpy.Message({'robotState': 11,
                                                       'web_session_id': randbytes(26).hex()}))
token = jm.call(req)['joystick_token']

joy_topic = roslibpy.Topic(ros, 'joystick_vel', 'mirMsgs/JoystickVel')


def drive(x, y, z):
    joy_topic.publish(roslibpy.Message({
        'speed_command': {
            'linear': {'y': y, 'x': x, 'z': 0.0},
            'angular': {'y': 0.0, 'x': 0.0, 'z': z}},
...
while ros.is_connected:
    if token is not None:
        ...
        if any(map(lambda x: abs(x) > 0.02, vel)):
            drive(-vel[1], vel[2], -vel[2])
            sleep(0.1)
            flag = True
        elif flag:
            flag = False
